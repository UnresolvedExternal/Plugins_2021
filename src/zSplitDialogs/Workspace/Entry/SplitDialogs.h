...
namespace NAMESPACE
{
	...
	public:
		...
		std::ostream& Npc(int index)
		{
			...
			return *npcs.insert({ index, std::make_unique<std::ofstream>(filePath) }).first->second;
		}
	};
...
#define NPC(index) Output::GetInstance().Npc(index)
		...
		void ExtractRecursive(int stackPos, int npc, std::vector<Replique>& repliques, std::vector<int>& poses)
		{
			...
			for (size_t i = 0; i < instructions.size(); i++)
			{
				...
				if (instructions[i].token == InstructionType::CallExternal && instructions[i].argument == ai_output)
				{
					...
					repliques += {};
					repliques.back().sourceNpc = sourceNpc;
					repliques.back().targetNpc = targetNpc;
					repliques.back().SetName(svmName);
				}
			}
		}
	...
	public:
		...
		{
			std::vector<int> indexes = { ai_output, self, other, hero, c_info, c_info_npc, c_info_information };
		...
		void Execute()
		{
			for (auto it = functions.begin(); it != functions.end(); it++)
			{
				...
				if (!TryGetRepliques(it->second, npc, repliques))
					continue;
				...
				NPC(npc) << std::endl;
				NPC(npc) << "\t\t\t" << it->second.GetSymbol()->name.ToChar() << std::endl;
