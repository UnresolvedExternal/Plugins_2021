namespace NAMESPACE
{
	...
	Hook<int(__thiscall*)(oCGame*, int), ActiveOption<int>> Ivk_oCGame_HandleEvent(ZENFOR(0x0065EEE0, 0x0068A300, 0x0069E980, 0x006FC170), &Hook_oCGame_HandleEvent, HookMode::Patch, Options::EventThrottling);
	...
	Hook<void(__thiscall*)(zCEventManager*, zCEventMessage*, zCVob*), ActiveOption<int>> Ivk_zCEventManager_OnMessage(ZENFOR(0x006DD090, 0x00715250, 0x00726940, 0x00786380), &Hook_zCEventManager_OnMessage, HookMode::Patch, Options::EventThrottling);
	void __fastcall Hook_zCEventManager_OnMessage(zCEventManager* _this, void* vtable, zCEventMessage* message, zCVob* sourceVob)
	{
		...
		if (inHandleEvent && messageMode == TMessageMode::Unknown && _this->hostVob == player && GetControlMessageCount(_this) >= Options::EventThrottling)
		{
