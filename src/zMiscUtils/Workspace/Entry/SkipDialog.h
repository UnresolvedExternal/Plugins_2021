namespace NAMESPACE
{
	...
	Hook<int(__thiscall*)(CGameManager*, int), ActiveOption<KeyCombo>> Ivk_CGameManager_HandleCancelKey(ZENFOR(0x00428D50, 0x0042B9A0, 0x0042A3E0, 0x0042A700), &Hook_CGameManager_HandleCancelKey, HookMode::Patch, Options::DiaHyperskipKey);
	int __fastcall Hook_CGameManager_HandleCancelKey(CGameManager* _this, void* vtable, int a0)
	{
		...
		if (!result || !Options::DiaHyperskipKey->GetPressed())
			return result;
