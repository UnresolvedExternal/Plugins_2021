namespace NAMESPACE
{
	...
	Sub clearDocsRead(ZSUB(GameEvent::Execute), []()
		{
			Options::TrackReadDocs.onChange += []()
			{
				if (!Options::TrackReadDocs)
					GetDocs().Clear();
			};
		});
	...
	Hook<int(__thiscall*)(oCItem*, int), ActiveValue<bool>> Ivk_oCItem_GetStateEffectFunc(ZENFOR(0x00672420, 0x0069FDE0, 0x006B4D80, 0x00712B80), &Hook_oCItem_GetStateEffectFunc, HookMode::Patch, Options::HookGetState);
	...
	Hook<int(__thiscall*)(oCNpc*, oCMsgManipulate*), ActiveOption<bool>> Ivk_oCNpc_EV_UseItemToState(ZENFOR(0x006AFC70, 0x006E3AF0, 0x006F6AC0, 0x007558F0), &Hook_oCNpc_EV_UseItemToState, HookMode::Patch, Options::TrackReadDocs);
	int __fastcall Hook_oCNpc_EV_UseItemToState(oCNpc* _this, void* vtable, oCMsgManipulate* message)
	{
		...
		if (getStateEffectCalledOn && getStateEffectState == 0 && _this->IsAPlayer() && message && (message->subType == oCMsgManipulate::EV_USEITEMTOSTATE || message->subType == oCMsgManipulate::EV_USEITEM) && message->targetVob == getStateEffectCalledOn)
		{
			oCItem* item = getStateEffectCalledOn;

			if (item->HasFlag(ITM_CAT_DOCS) && item->objectName.Length())
			{
				if (!parser->GetSymbol(item->GetInstance()))
				{
					cmd << endl;
					cmd << "zMarkItems: invalid document" << endl;
					LogItem(item);
					return result;
				}
