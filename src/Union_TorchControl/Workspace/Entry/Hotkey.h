namespace NAMESPACE
{
	bool GetToggled()
	{
		if (*Settings::TorchHotkeyOverride)
			return Settings::TorchHotkeyOverride->GetToggled();

		if (Settings::TorchHotkeyLogicalValue)
			return zinput->GetToggled(Settings::TorchHotkeyLogicalValue);
	...
	Sub useTorch(ZSUB(GameEvent::Loop), Settings::EnableHotkey, []()
		{
	...
	void BindTorchOptionKey(zCInput* _this, int mode)
	{
		...
		_this->BindOption("keyTorch", Settings::TorchHotkeyLogicalValue, ctrls);
	}

	void __fastcall Hook_zCInput_BindKeys(zCInput*, void*, int);
	Hook<void(__thiscall*)(zCInput*, int), ActiveOption<int>> Ivk_zCInput_BindKeys(ZENFOR(0x004C5930, 0x004D5420, 0x004CD890, 0x004CFE00), &Hook_zCInput_BindKeys, HookMode::Patch, Settings::TorchHotkeyLogicalValue);
	...
	Sub bindKey(ZSUB(GameEvent::Execute), []()
		{
			static int lastValue = Settings::TorchHotkeyLogicalValue;

			Settings::TorchHotkeyLogicalValue.onChange += []()
			{
				if (Settings::TorchHotkeyLogicalValue == lastValue)
					return;
				...
				lastValue = Settings::TorchHotkeyLogicalValue;
			};
		});
}